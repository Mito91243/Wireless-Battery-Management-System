...
inline void initBQ76952() {
  ...
  bms.writeByteToMemory(0x92FD, 0x07); // TS1 = NTC10K + 18kΩ
  ...
  bms.writeByteToMemory(0x92FA, 0x07); // CFETOFF = NTC10K thermistor input
  delay(50);
  ...
  // CFETOFF configured as thermistor (done in section 3 above)
  // DFETOFF: Not used on main board, disable
  ...
  currentBalMode = (BalancingMode)prefs.getInt("bal_mode", BAL_MODE_HOST_ALGO);
  ...
  // Bits 1-0: USER_AMPS_1:0 = 0 (1mA units for current)
