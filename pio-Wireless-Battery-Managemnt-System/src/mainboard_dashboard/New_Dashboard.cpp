...
void readBMSData() {
  ...
  temp1 = bms.getThermistorTemp(TS1); // TS1 thermistor
  temp2 = bms.getThermistorTemp(HDQ); // HDQ thermistor
  temp3 = bms.getThermistorTemp(TS3); // TS3 thermistor
...
void hostBalancingLoop() {
  ...
  if (!balancingEnabled) {
    ...
    if (hostBalancingActive) {
      ...
      // Helper to map logical battery cell index to BQ76952 balancing bit
      auto getPinForCell = [](int cellIndex) -> int {
        return CELL_TO_BAL_BIT[cellIndex];
      };
...
void setup() {
  ...
  // (HIGH) to light it. (Testboard)
  // FOR MAINBOARD: The BQ Open-Drain pulls it LOW. We just read it as an input!
  pinMode(TB_PIN_ALERT, INPUT);
  Serial.printf("[TB-Dash] ALERT LED: GPIO%d set to INPUT. "
                "Watching for BQ heartbeat.\n",
                TB_PIN_ALERT);
...
void loop() {
  ...
  // 1. HARDWARE ALERT PIN POLLING
  // Instead of polling I2C, we watch the ALERT pin. The BQ76952 drives it
  // LOW (Open-Drain) when an ADC sweep finishes or a fault occurs.
  static unsigned long lastAlarmPoll = 0;
  if (millis() - lastAlarmPoll >= 50) { // Fast polling for responsiveness
    lastAlarmPoll = millis();
    bool alertActive = (digitalRead(TB_PIN_ALERT) == LOW);
    ...
    // alertActive = BQ pulled pin LOW (data ready or fault)
    else if (alertActive || (millis() - lastRead > 2000)) {
      lastRead = millis();
      t0 = micros();
      readBMSData();
      perf_i2c_us = micros() - t0;
