 ...
 *   1. Removed duplicate COSCMD_ALL_FETS_ON define (was on two lines)
 *   2. Removed fragile incremental computeChecksum(); replaced with
 *      inline uint16_t sum pattern in .cpp (matches subCommandWriteData style)
 */
...
#define COSCMD_SLEEP_ENABLE 0x0099      // Enable SLEEP mode (device sleeps when current is low)
#define COSCMD_SLEEP_DISABLE 0x009A     // Disable SLEEP mode (also wakes from SLEEP)
#define COSCMD_DEEP_SLEEP 0x000F        // Enters Deep Sleep mode (must send twice in 4s)
#define COSCMD_EXIT_DEEP_SLEEP 0x000E   // Exit Deep Sleep mode
#define COSCMD_SHUTDOWN 0x0010           // Enters Shutdown mode (must send twice in 4s if sealed)
