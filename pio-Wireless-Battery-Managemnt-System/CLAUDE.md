...
BQ76952 BMS IC ──(I2C)──► Slave ESP32 ──(ESP-NOW)──► Master ESP32 ──(MQTT/WiFi)──► Cloud Broker
  (16-cell monitor)         sender.cpp                  reciever.cpp            wbms.systems:1883
```
...
6. Cloud broker at `wbms.systems:1883` receives the data for dashboarding/storage
...
| Master <-> Cloud | MQTT | Broker at `wbms.systems:1883`, topic `bms/data`, client ID `wbms-master` |
...
| MQTT Broker | `MQTT_BROKER` | `"wbms.systems"` | Plain MQTT, no TLS |
